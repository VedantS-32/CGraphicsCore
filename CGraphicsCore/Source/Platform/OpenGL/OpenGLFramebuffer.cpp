...
namespace Cgr
{
	...
	namespace Utils
	{
		...
		static GLenum CGRTextureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			CGR_CORE_ASSERT(m_ColorAttachments.size() <= 4);
	...
	void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
	{
		...
		{
			CGR_CORE_WARN("Attempted to resize framebuffer to {0}, {1}", width, height);
			return;
		}
	...
	{
		CGR_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());
	...
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
			Utils::CGRTextureFormatToGL(spec), GL_INT, &value);
	}
}
