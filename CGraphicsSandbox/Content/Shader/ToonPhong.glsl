...
void main()
{
	...
	vec3 lightDir = normalize(vWorldSettings.LightPosition);
	vec3 viewDir = normalize(vWorldSettings.CameraPosition);
	...
	float specular = pow(max(dot(normal, halfVector), 0.0), vMaterialParams.SpecularAlpha);
	float toonSpecular = step(0.5, specular); // Simple binary specular highlight

	// Ambient color
	vec4 ambient = vec4(vWorldSettings.AmbientColor, 1.0) * vec4(texture(uTextures[0], vModelProps.TexCoord).rgb, 1.0);
